...
/**
 * Disable Gutenberg Editor for the impacted post types
 */

function wpwcl_use_gutenberg($is_enabled, $post_type) {
	$options = get_option( 'wpwcl_settings_options' );
	...
	if (in_array( $post_type, $imp_p_types, true ) || !post_type_supports( $post_type, 'editor' )) {
		$can_edit = false;
...
add_filter('gutenberg_can_edit_post_type', 'wpwcl_use_gutenberg', 10, 2); // For old Gutenberg version
add_filter('use_block_editor_for_post_type', 'wpwcl_use_gutenberg', 10, 2);
